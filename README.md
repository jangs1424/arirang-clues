...
`qr/qr-spare.pdf` — A4 1장, 30칸(5열 × 6행). 칸 크기는 정본과 같으므로 그대로 덮인다.

집 색도 이름도 없는 검정 무지다. 어느 가구 카드에든 붙일 수 있어야 하기 때문이고,
번호(`S01`~`S30`)만 찍혀 있다. 이 번호가 어디로 갈지는 종이가 아니라 `spares.json` 이 정한다.

```
# spares.json 에서 붙인 번호의 값을 채운다
#   ""           -> "준비 중입니다" 안내
#   "P-3"        -> 그 단서카드 화면 (/c/P-3/ 과 완전히 동일)
#   "https://…"  -> 그 주소로 리다이렉트
python3 build_spares.py
git add -A && git commit -m "예비 QR 연결" && git push
```

**목적지를 몇 번을 바꿔도 이미 붙인 스티커는 그대로 쓴다** — QR 이 가리키는 `/s/<번호>/` 는 고정이다.
값에 오타가 있으면 아무것도 쓰지 않고 멈춘다. 일부만 반영된 상태가 생기지 않게 한 것이다.
